...
PLAN_PHASE_FIELDS = ("Goal", "Scope", "Dependencies", "Parallelizable", "Success definition", "Notes")
...
def setup_project(args: argparse.Namespace) -> int:
    ...
    (_ideas_dir(project_root)).mkdir(parents=True, exist_ok=True)
    (_features_dir(project_root)).mkdir(parents=True, exist_ok=True)
    _write(_ideas_dir(project_root) / ".gitkeep", "")
    _write(_features_dir(project_root) / ".gitkeep", "")
    if getattr(args, "export_cursor", False):
        export_cursor(argparse.Namespace(project=str(project_root)))
    print(runtime_dir)
    return 0
...
def new_idea(args: argparse.Namespace) -> int:
    project_root = _project_root(args.project)
    setup_project(argparse.Namespace(project=str(project_root)))
...
def _check_feature(feature_dir: Path) -> list[str]:
    ...
    for label in ("Initial phase proposal", "User-confirmed success definition", "User-confirmed phase ordering", "Cost or scope tradeoffs discussed"):
